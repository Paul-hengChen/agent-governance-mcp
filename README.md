...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.2.0
...
Every client below points at the same command: `npx -y github:Paul-hengChen/agent-governance-mcp#v3.2.0`. Pick the section that matches your tool.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.2.0
claude mcp list
# agent-governance-mcp: npx -y github:Paul-hengChen/agent-governance-mcp#v3.2.0 - ✓ Connected
```
...
```json
{
  "mcpServers": {
    "agent-governance-mcp": {
      "command": "npx",
      "args": ["-y", "github:Paul-hengChen/agent-governance-mcp#v3.2.0"]
    }
  }
}
```
...
mcpServers:
  - name: agent-governance-mcp
    ...
    args:
      - "-y"
      - "github:Paul-hengChen/agent-governance-mcp#v3.2.0"
```
...
**Google Antigravity**: open the in-app MCP Server settings UI and add a new entry — command `npx`, args `-y github:Paul-hengChen/agent-governance-mcp#v3.2.0`. (Underlying config file is platform-dependent.)
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.2.0 agent-governance-context",
            "timeout": 60
          }
        ]
      }
...
npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.2.0 agent-governance-context
...
```bash
# Local
npx -y github:Paul-hengChen/agent-governance-mcp#v3.2.0 --port 3000 --db ./teamwork.db
# Liveness probe
...
TW_AUTH_TOKEN=hunter2 TW_ALLOWED_ORIGINS=https://app.example.com \
  npx -y github:Paul-hengChen/agent-governance-mcp#v3.2.0 --port 3000
```
...
docker run --rm -p 3000:3000 \
  ...
  -v $(pwd)/data:/app/data \
  agent-governance-mcp --db /app/data/teamwork.db
```
