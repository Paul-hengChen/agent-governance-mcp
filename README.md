...
> **Status**: production-used, v3.92.1. Suite **1618/1618**. Stdio mode is solo/single-machine; HTTP+SQLite mode is for multi-machine teams.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.92.1
...
npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.92.1 agc init
...
```json
{
  "hooks": {
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.92.1 agent-governance-context",
        "timeout": 60
      }]
    }]
  }
