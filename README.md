...
Server 架構不綁任何特定流程。Speckit 是預設的 task 格式，但**完全可換**：

- **Task 格式自訂**：放 `.current/.config.json` 指定 `taskPattern` (regex) + `taskPaths`
- **憲法 / Skill 自訂**：在 `.current/constitution.md` / `.current/skill-sr-engineer.md` 放你自己的版本，server 會優先用 workspace 的覆蓋而不是內建預設
- **純 vibe coding 模式**：完全不需要 `tasks.md`，4 個 task 工具會優雅回 "No tasks.md found"，handoff state + prompt 注入照常運作

範例 — 換成 JIRA-style task ID：
```jsonc
...
```
正則的合約：group 1 是 checkmark (`" "` 或 `"x"`)，group 2 是 task ID。其餘 group 會被串成 description。Task 行本身仍需用標準 markdown checkbox 語法 (`- [ ]` / `- [x]`)，因為 complete/rollback 用此語法做原地翻轉。
