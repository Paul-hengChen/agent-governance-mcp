...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.14.1
...
npx -y --package=github:Paul-hengChen/agent-governance-mcp#v3.14.1 agc init
...
Every client below points at the same command: `npx -y github:Paul-hengChen/agent-governance-mcp#v3.14.1`. Pick the section that matches your tool.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.14.1
claude mcp list
# agent-governance-mcp: npx -y github:Paul-hengChen/agent-governance-mcp#v3.14.1 - ✓ Connected
```
...
```json
{
  "mcpServers": {
    "agent-governance-mcp": {
      "command": "npx",
      "args": ["-y", "github:Paul-hengChen/agent-governance-mcp#v3.14.1"]
    }
  }
}
```
...
mcpServers:
  - name: agent-governance-mcp
    ...
    args:
      - "-y"
      - "github:Paul-hengChen/agent-governance-mcp#v3.14.1"
```
...
**Google Antigravity**: open the in-app MCP Server settings UI and add a new entry — command `npx`, args `-y github:Paul-hengChen/agent-governance-mcp#v3.14.1`. (Underlying config file is platform-dependent.)
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.14.1 agent-governance-context",
            "timeout": 60
          }
        ]
      }
...
npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.14.1 agent-governance-context
...
```bash
# Local
npx -y github:Paul-hengChen/agent-governance-mcp#v3.14.1 --port 3000 --db ./agc.db
# Liveness probe
...
TW_AUTH_TOKEN=hunter2 TW_ALLOWED_ORIGINS=https://app.example.com \
  npx -y github:Paul-hengChen/agent-governance-mcp#v3.14.1 --port 3000
```
