...
### ✅ Pros
1. **Zero-Config Startup**: Run directly via `npx github:...#v3.7.4` (version-pinned). No *manual* clone or install — first invocation auto-downloads dependencies (~30-60s), subsequent runs are instant from the npx cache.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.7.4
...
npx -y --package=github:Paul-hengChen/agent-governance-mcp#v3.7.4 agc init
...
Every client below points at the same command: `npx -y github:Paul-hengChen/agent-governance-mcp#v3.7.4`. Pick the section that matches your tool.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.7.4
claude mcp list
# agent-governance-mcp: npx -y github:Paul-hengChen/agent-governance-mcp#v3.7.4 - ✓ Connected
```
...
```json
{
  "mcpServers": {
    "agent-governance-mcp": {
      "command": "npx",
      "args": ["-y", "github:Paul-hengChen/agent-governance-mcp#v3.7.4"]
    }
  }
}
```
...
mcpServers:
  - name: agent-governance-mcp
    ...
    args:
      - "-y"
      - "github:Paul-hengChen/agent-governance-mcp#v3.7.4"
```
...
**Google Antigravity**: open the in-app MCP Server settings UI and add a new entry — command `npx`, args `-y github:Paul-hengChen/agent-governance-mcp#v3.7.4`. (Underlying config file is platform-dependent.)
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.7.4 agent-governance-context",
            "timeout": 60
          }
        ]
      }
...
npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.7.4 agent-governance-context
...
```bash
# Local
npx -y github:Paul-hengChen/agent-governance-mcp#v3.7.4 --port 3000 --db ./agc.db
# Liveness probe
...
TW_AUTH_TOKEN=hunter2 TW_ALLOWED_ORIGINS=https://app.example.com \
  npx -y github:Paul-hengChen/agent-governance-mcp#v3.7.4 --port 3000
```
...
A: Zero config. Your team doesn't need to clone or install anything. The install command pins a specific tag (`#v3.7.4`) so upstream breaking changes can't silently affect your sessions — see the next question for how to upgrade.

**Q: How do I upgrade, or pin to a different version?**
A: Replace `#v3.7.4` in the install command with another tag or `#main` to track the bleeding edge. After changing it, clear the npx cache: `npx clear-npx-cache`. The CHANGELOG records breaking changes per version.
