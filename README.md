...
### ✅ Pros
1. **Zero-Config Startup**: Run directly via `npx github:...`. No *manual* clone or install — first invocation auto-downloads dependencies (~30-60s), subsequent runs are instant from the npx cache.
...
**Google Antigravity**: open the in-app MCP Server settings UI and add a new entry — command `npx`, args `-y github:Paul-hengChen/teamwork-mcp-server`. (Underlying config file is platform-dependent.)
...
npx -y -p github:Paul-hengChen/teamwork-mcp-server teamwork-context
