...
#### (u) PRD-split guardrails — Feature-Scope Gate + design-auditor Volume Gate (v3.17.0, 2026-05-31)
MINOR release adding two complementary front-door guardrails so large, design-heavy PRDs can't overrun the design-auditor.

- **Feature-Scope Gate** (`skill-coordinator.md`) — new SOP step (after state-sync, before Design-source detection) that judges **text-only** (never fetching a design) whether a PRD is one feature or many. Single → automation continues; multi → STOP, write `.current/feature-split.md` (coordinator pre-fills all but `figma link` + `notes / 注意事項`), recommend a split, wait for the human. Lite unaffected; always-on footprint ~350 tok.
- **design-auditor Volume Gate + node-scoped fetch** (`skill-design-auditor.md`) — pre-fetch input-side gate (fetch modes only) that STOPs (`Blocked → pm`, fail-loud) when a single feature's design source exceeds ~one feature's worth, recommending a further split instead of ingest-then-defer; plus node-scoped fetch (pull only the frames audited this pass). The split-schema asks for **frame-scoped** Figma links to bound the fetch at the source.
- Both are prompt-layer + human-checkpoint (advisory; no transition-matrix change). Suite **432/432**.

Spec: `specs/feature-scope-gate.md`, `specs/design-auditor-volume-guard.md`.

#### (v) Feature-split lifecycle — status + resume + by-id (v3.18.0, 2026-05-31)
MINOR release giving `.current/feature-split.md` a lifecycle so a split plan resumes safely across `/teamwork` runs without redoing completed units.

- **`status` column** (`pending`/`done`) in the Split Table, pre-filled `pending`.
- **Resume + done-marking** — an existing plan is no longer re-assessed/regenerated; the gate **reconciles** (a row flips to `done` when its `feature id` matches the handoff `active_feature` at PASS), then works the next `pending` row — or a human-named row (`do F0` / a feature id) — by **hydrating** it (scope + figma + widgets + notes). A `done` row is never re-run.
- Prompt-layer + human-checkpoint; no transition-matrix change. Footprint ceiling raised ~425 → ~550 tok (section ~496). Suite **439/439**.

Spec: `specs/feature-split-lifecycle.md`.
...
Every client below points at the same command: `npx -y github:Paul-hengChen/agent-governance-mcp#v3.18.0`. Pick the section that matches your tool.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.18.0
claude mcp list
# agent-governance-mcp: npx -y github:Paul-hengChen/agent-governance-mcp#v3.18.0 - ✓ Connected
```
...
**Google Antigravity**: open the in-app MCP Server settings UI and add a new entry — command `npx`, args `-y github:Paul-hengChen/agent-governance-mcp#v3.18.0`. (Underlying config file is platform-dependent.)
...
TW_AUTH_TOKEN=hunter2 TW_ALLOWED_ORIGINS=https://app.example.com \
  npx -y github:Paul-hengChen/agent-governance-mcp#v3.18.0 --port 3000
```
