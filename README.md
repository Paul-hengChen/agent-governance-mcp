...
> **Status**: production-used, v3.80.0. Suite **1390/1390**. Stdio mode is solo/single-machine; HTTP+SQLite mode is for multi-machine teams.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.80.0
...
npx -y github:Paul-hengChen/agent-governance-mcp#v3.80.0 agc init
...
```json
{
  "hooks": {
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.80.0 agent-governance-context",
        "timeout": 60
      }]
    }]
  }
