...
> **Status**: production-used, v3.91.0. Suite **1611/1611**. Stdio mode is solo/single-machine; HTTP+SQLite mode is for multi-machine teams.
...
claude mcp add -s user agent-governance-mcp -- npx -y github:Paul-hengChen/agent-governance-mcp#v3.91.0
...
```json
{
  "hooks": {
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "npx -y -p github:Paul-hengChen/agent-governance-mcp#v3.91.0 agent-governance-context",
        "timeout": 60
      }]
    }]
  }
