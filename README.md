...
### ✅ Pros
1. **Zero-Config Startup**: Run directly via `npx github:...#v3.1.0` (version-pinned). No *manual* clone or install — first invocation auto-downloads dependencies (~30-60s), subsequent runs are instant from the npx cache.
...
claude mcp add -s user teamwork-mcp-server -- npx -y github:Paul-hengChen/teamwork-mcp-server#v3.1.0
...
Every client below points at the same command: `npx -y github:Paul-hengChen/teamwork-mcp-server#v3.1.0`. Pick the section that matches your tool.
...
claude mcp add -s user teamwork-mcp-server -- npx -y github:Paul-hengChen/teamwork-mcp-server#v3.1.0
claude mcp list
# teamwork-mcp-server: npx -y github:Paul-hengChen/teamwork-mcp-server#v3.1.0 - ✓ Connected
```
...
```json
{
  "mcpServers": {
    "teamwork-mcp-server": {
      "command": "npx",
      "args": ["-y", "github:Paul-hengChen/teamwork-mcp-server#v3.1.0"]
    }
  }
}
```
...
mcpServers:
  - name: teamwork-mcp-server
    ...
    args:
      - "-y"
      - "github:Paul-hengChen/teamwork-mcp-server#v3.1.0"
```
...
**Google Antigravity**: open the in-app MCP Server settings UI and add a new entry — command `npx`, args `-y github:Paul-hengChen/teamwork-mcp-server#v3.1.0`. (Underlying config file is platform-dependent.)
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "npx -y -p github:Paul-hengChen/teamwork-mcp-server#v3.1.0 teamwork-context",
            "timeout": 60
          }
        ]
      }
...
npx -y -p github:Paul-hengChen/teamwork-mcp-server#v3.1.0 teamwork-context
...
```bash
# Local
npx -y github:Paul-hengChen/teamwork-mcp-server#v3.1.0 --port 3000 --db ./teamwork.db
# Liveness probe
...
TW_AUTH_TOKEN=hunter2 TW_ALLOWED_ORIGINS=https://app.example.com \
  npx -y github:Paul-hengChen/teamwork-mcp-server#v3.1.0 --port 3000
```
...
A: Zero config. Your team doesn't need to clone or install anything. The install command pins a specific tag (`#v3.1.0`) so upstream breaking changes can't silently affect your sessions — see the next question for how to upgrade.

**Q: How do I upgrade, or pin to a different version?**
A: Replace `#v3.1.0` in the install command with another tag (`#v3.2.0` once released) or `#main` to track the bleeding edge. After changing it, clear the npx cache: `npx clear-npx-cache`. The CHANGELOG records breaking changes per version.
