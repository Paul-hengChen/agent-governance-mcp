...
Before any diff, the impl capture MUST be in the SAME state the baseline depicts — each `## Visual Baselines` row carries a `canonical state` (selected item / focused row / scroll offset / drawer-or-modal open / toggle + segmented values / expected default data). Emit one checkbox row per surface under H2 `## Canonical State Verification`. A `[ ]` (state mismatch) is a **capture defect**, NOT visual drift: do NOT diff or "accept" it — recapture the impl in the baseline's state, or FAIL; unresolved, it blocks PASS (server-checked). Reaching the canonical state is the engineer's/QA's job. **Multi-value guard:** if a property has MORE than one correct value depending on context (focused vs unfocused, selected vs unselected), you MUST NOT adjudicate one value as "correct" — record BOTH contexts as separate baselines (or flag the surface for re-audit with per-context baselines) and FAIL it with note: "context-dependent property requires per-context baseline — see §四#7 in `research/mode-feature-process-retrospective.md`". Per Constitution §3.2, single-choice adjudication of a multi-value property is a **specification ambiguity**, not an implementation defect — and stays `FAIL` per this step: no approved divergence is on record here, so the `Blocked` contract-defect route (`content/skill-qa-engineer.md` § *Contract Defect vs Implementation Failure*) does not apply.

### Actual-Capture Output Convention (E30)

Actual (impl) screenshot captures — yours and any consumer test suite's (`ACTUAL_DIR` or equivalent) — go to an **UNTRACKED directory outside the committed baseline dir** (e.g. a `.gitignore`d `__actual__/` sibling or a tmp dir), NEVER into the baseline dir itself. Actuals written next to committed baselines dirty git status on every run, making a real visual regression indistinguishable from routine overwrite at a glance (this once cost a full pixelmatch forensics round to clear a false alarm). QA flags any suite whose actual-capture output violates this convention as a finding in the visual report.
...
A row where BOTH `pass` and `fail` would be dishonest is not a Structural Assertion failure — the decision test and escalation route live in `content/skill-qa-engineer.md` § *Contract Defect vs Implementation Failure*, not here.
...
| Widget shape miss — any unchecked Step A row | `VISUAL_WIDGETS_UNVERIFIED` | `tw_rollback_task(<task-id>, "QA: Visual Widgets shape miss")` → `tw_update_state(status=FAIL, agent_id="qa-engineer", qa_review=<list of missing widgets>, next_role="sr-engineer", pending_notes=["QA: <task-id> Phase 1.5 FAIL — widget shape miss", "visual_fail: <widget-id-list>"])`. STOP. |
| Pixel drift — ≥ 1 material Step B difference, all widget shapes verified | — | `tw_rollback_task(<task-id>, "QA: Phase 1.5 pixel drift")` → `tw_update_state(status=FAIL, agent_id="qa-engineer", qa_review=<diff>, next_role="sr-engineer", pending_notes=["QA: <task-id> Phase 1.5 FAIL — pixel drift", "visual_fail: pixel"])`. STOP. |
| Missing baseline file on disk | — | `tw_update_state(status=FAIL, agent_id="qa-engineer", qa_review=<path>, next_role="design-auditor", pending_notes=["QA: missing baseline — <path>"])`. STOP. Design-auditor defect — no `visual_fail:` prefix. |
| Missing impl screenshot | — | `tw_update_state(status=FAIL, agent_id="qa-engineer", qa_review=<path>, next_role="sr-engineer", pending_notes=["QA: missing impl screenshot — <path>", "visual_fail: missing_impl"])`. STOP. |
| Dimension/scale mismatch — tool reports `dimensionsMatch=false` (e.g. baseline exported @1× but impl captured @2×) | `VISUAL_PROVENANCE_MISSING` if recorded as the diff-metric | Hard FAIL — NOT a graceful skip, NOT an Allowed Difference; the surface MUST NOT be recorded `pass`/`accepted`. Re-export the baseline at the correct scale and re-run the diff; if it cannot be re-exported now, take the missing-baseline path with `next_role="design-auditor"` and `pending_notes=["QA: missing baseline — dimension mismatch @Nx vs @Nx <surface>"]`. STOP. |
| Step A.0: Source manifest or its frozen node-id list absent | — | `tw_update_state(status=FAIL, agent_id="qa-engineer", qa_review="Source manifest missing frozen baseline node-id list", next_role="design-auditor", pending_notes=["QA: baseline node-id manifest absent — re-derivation forbidden"])`. STOP. |
