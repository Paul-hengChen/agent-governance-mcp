...
| last write is `status: PASS` | PASS (terminal) | terminal success — release-engineer is a deliberate human decision, not an auto-hop; after surfacing, run the *Backlog Intake Loop* below for the NEXT ticket (the loop never auto-hops to release-engineer) | human |
...
| **Claim-vs-state mismatch**<!-- origin:start --> (v3.103.0, E72)<!-- origin:end --> — after EVERY handoff, `tw_get_state` and diff the finished role's claims (`completed_tasks`, `pending_notes`, the evidence file it names) against what landed; a reply asserting content the write did not persist fires this row. No gate catches this class — the server validates a write against rules, never against what its author said it was doing | — | do not route on the reply — Crash-Resume-class: ground-truth first, then re-dispatch with the mismatch stated in the brief | (role being re-dispatched) |
| **Cut-approval gate** — the `next_role` field is `architect` or `sr-engineer` but `cut_approved` is not set on the handoff (server error: `CUT_APPROVAL_REQUIRED`) | — | surface the cut draft and wait — do NOT auto-hop through to build; writer obligation below (sole exception: a cut qualifying for the Constitution §3.1 Cut-Approval Auto-Tier — auto-tier writer action below, no halt) | human |
...
| **Source-credibility gate** — the `next_role` field is `architect` or `sr-engineer` but a fetch-based `design/<feature>.md` (`## Mode` ∈ `figma`/`sketch`/`xd`/`penpot`) has an `audited` `## Source` row lacking `credibility: full-page-composite` (server error: `SOURCE_CREDIBILITY_UNVERIFIED`) | — | surface the offending row(s) from the rejection envelope and wait — do NOT auto-hop through to build; the design-auditor must re-run step 2b Source-Credibility Classification and record the attestation before build | human |
...
**Known non-mismatches**<!-- origin:start --> (v3.103.0, E72)<!-- origin:end -->: the code-reviewer's APPROVED handoff legitimately stamps `agent_id="qa-engineer"` (Constitution §3.1's canonical edge, spelled out as `skill-code-reviewer.md`'s APPROVED row) and that edge legitimately resets `review_round` to 0. Neither is an identity misstamp — do not fire the row above on them.
...
**Auto-tier writer action**<!-- origin:start --> (v3.85.0, E5)<!-- origin:end --> (mechanism, threshold, and arming key: Constitution §3.1 Cut-Approval Auto-Tier): when the tier is armed (`.current/.config.json` has `cutApprovalAutoTier`) and the presented cut meets EVERY threshold condition, the same sanctioned writer sets `cut_approved: true` WITHOUT halting for the human, and MUST record the tier in the SAME write: a `pending_notes` line `cut-approved: auto-tier` plus the threshold facts (file count, ticket priority, no-schema-change, non-design). Over-threshold, design-armed, or unclassifiable cuts halt exactly as the writer obligation above — when in doubt, halt.

## Backlog Intake Loop<!-- origin:start --> (v3.85.0, E5)<!-- origin:end -->

At feature close — the *PASS* stop-condition row fired, or a human-decided release just completed — do NOT end unconditionally with "next feature is a human decision":

1. Read the workspace backlog in its own declared order (`docs/backlog.md` execution order / its "Remaining open tickets" list; skip done-marked rows). No backlog file → stop as before (human decision).
2. Take the next open ticket and run the SOP *Cheapest-Compliant-Path Intake* step on it.
3. **Auto-start** ONLY when the ticket's cut qualifies for the Constitution §3.1 Cut-Approval Auto-Tier (tier armed AND every threshold condition met): start it on its intake classification, recording the approval via the *Auto-tier writer action* above.
4. Otherwise **auto-propose**: surface the ticket + its one-line intake classification and wait for the human.

Bounds: PASS stays terminal for the CURRENT feature — release-engineer remains a deliberate human decision and this loop NEVER auto-hops to release-engineer; the loop governs only what the coordinator surfaces or starts NEXT. The feature lease, the hop cap, and every other Escalation Routes row apply unchanged inside the started ticket.
