...
- **Version-assertion tests are self-updating, not release-engineer's to edit**: `test/baseline-manifest-gate.test.mjs` and `test/pixel-gate-attestation.test.mjs`'s AC-9 tests read `package.json`/`index.ts` dynamically (no hardcoded target version) — a version bump (SOP step 4) requires ZERO test-file edits. If `npm test` (SOP step 6) ever fails on a version-literal mismatch, a new hardcoded-version assertion was introduced somewhere — do NOT hand-edit it yourself (Constitution §2); STOP and route to qa-engineer via `pending_notes=["release-engineer: found hardcoded version literal in <test file> — qa-engineer must retarget or relax it", "next_role: qa-engineer"]`.
