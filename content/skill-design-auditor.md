...
- **Token-frugal multi-pass**: output is bounded by the `pass_budget` (constitution §5 anti-loop) — at most its per-pass line ceiling each pass, up to its pass count per feature. For designs that cannot fit in one pass, run additional passes (each within the per-pass ceiling) that append Copy / Strings + Visual Tokens rows to the same `design/<feature>.md`. Each subsequent pass MUST flip ≥ 1 *Source manifest* row from `deferred` → `audited` — passes that make no manifest progress are forbidden.
...
- **Source manifest** — exhaustive list of every surface in the design source. Emit it as a header-bearing `## Source` H2 table so downstream parsers can locate columns by name; one row per surface: `<medium> | <pointer> | <fetched? yes/no> | <status: audited \| deferred \| out-of-scope> | <credibility> | <reason>`. Pointer is a Figma node id, Sketch artboard id, XD artboard id, Penpot board id, PDF page number, image filename, photo filename, etc. Manifest MUST cover every frame / artboard / board / page / file in the source — not just the ones referenced by the current task. `reason` is required for `deferred` and `out-of-scope`; optional for `audited`. **`credibility` column (E4)** — for fetch-based modes (`figma`/`sketch`/`xd`/`penpot`) every `audited` row MUST carry the literal `full-page-composite`, recorded by step 2b's Source-Credibility Classification below; `deferred` / `out-of-scope` rows leave it blank (they never reach the gate). `full-page-composite` is the ONLY value an `audited` row can legally carry — classifications (b)/(c)/(d) STOP before any row is written (step 2b) — and the server verifies it at the pm→build hop (`SOURCE_CREDIBILITY_UNVERIFIED`, see step 2b). **Backwards-compat**: pre-Phase-1 audits lacking the status column are treated by downstream roles as `audited` for surfaces they list and `unknown` for the rest — no retroactive migration; a fetch-based design with no `## Source` section at all is dormant for this gate (no backfill).
...
2a. **Volume Gate (pre-fetch)** — fetch-based modes (`figma`/`sketch`/`xd`/`penpot`) only. BEFORE extracting, estimate the source's surface/frame count via cheap metadata (frame list / node count) — NOT a full-document fetch. WHEN a single feature's source exceeds roughly one feature's worth — more surfaces than the `pass_budget` could audit, OR a fetch that would dominate the context budget — DO STOP per *Escalation Routes: design source oversized* (`status=Blocked` → `next_role=pm`). ELSE continue to extraction. Do NOT ingest-then-defer — splitting the feature is preferred to overflowing context. `image`/`pdf`/`paper`/`no-design` skip this gate (human-confirmed values, no bulk fetch). This is the input-side mirror of the `pass_budget` output cap.
2b. **Source-Credibility Classification<!-- origin:start --> (v3.38.0)<!-- origin:end -->** — fetch-based modes only (`figma`/`sketch`/`xd`/`penpot`). BEFORE extracting any values, classify each target node into one of:
   ...
   WHEN the classification is (b), (c), or (d) — i.e. NOT a full-page/screen composite frame for the intended feature — you MUST STOP per *Escalation Routes: node type mismatch* (`status=Blocked`, `next_role=pm`, unchanged). ELSE proceed to extraction. Do NOT transcribe values from the wrong node type; the guardrail fires BEFORE any values are written to the audit artifact. P2 was saved by this behaviour; P1 was reopened for lack of it (see `research/mode-feature-process-retrospective.md` §四#2). `image`/`pdf`/`paper`/`no-design` modes skip this gate (human-confirmed sources).
   - **Record the attestation (E4).** WHEN the classification is (a) — and only (a) reaches an `audited` row, because (b)/(c)/(d) STOP above — record `credibility: full-page-composite` in that node's `audited` row of the *Source manifest* (§Artifact Schema). This turns the already-mandatory step-2b classification into a durable, machine-checkable artifact. **Server-enforced at the pm→build hop:** for a fetch-based design (`figma`/`sketch`/`xd`/`penpot`), any `audited` `## Source` row whose `credibility` cell is empty, absent, or not the literal `full-page-composite` blocks the `pm:In_Progress → {architect,sr-engineer}:In_Progress` transition with `SOURCE_CREDIBILITY_UNVERIFIED`. Dormant for `image`/`pdf`/`paper`/`no-design` modes, and when the design has no `## Source` section at all (no backfill). See `specs/e4-design-source-credibility-gate.md`.
2c. **Mechanical baseline selection (v3.39.0)** — fetch-based modes only (`figma`/`sketch`/`xd`/`penpot`), and ONLY when the source is a multi-surface board (one URL/node expands to many surfaces, e.g. a full OOBE flow board) and the task needs a subset as baselines. Do NOT eyeball-scan the board and hand-pick frames — that is unreproducible, unaccountable, and varies per run (漏抓 / 誤收 / 把箭頭·標註當畫面). Instead select via a **deterministic structural filter** over the metadata you already pulled in 2a, written so a human can re-run it:
   ...
   - **Server-enforced at PASS (<!-- origin:start -->v3.40.0 baseline manifest gate, <!-- origin:end -->Constitution §3.1).** This step's artifacts are no longer prose-only — they are now machine-checked. The *Source manifest* MUST be a `## Source` H2 table whose selected rows carry `status: audited` and a non-empty node-id `pointer`; a manifest with zero audited rows (or no `## Source` section once the design is on the manifest contract) blocks PASS with `BASELINE_MANIFEST_MISSING`. For **multi-surface** selections (≥2 audited rows) the filter conditions + exclusion reasons MUST be recorded in a dedicated **`## Baseline Selection Provenance`** H2 section containing both a `filter-conditions:` line AND an `exclusion-reasons:` line; omitting either blocks PASS with `BASELINE_PROVENANCE_INCOMPLETE`. Single-surface selections (exactly 1 audited row) are exempt from the provenance section. Section/label format:
     ...
     - exclusion-reasons: dropped CONNECTOR/annotation nodes; dropped deferred surfaces (over the pass_budget per-pass cap)
     ```
...
6. Hand off per *Escalation Routes: audit complete* (include `active_feature=<name>` on the write).
