...
3b. **Repro-First (bugfix mode)**<!-- origin:start --> (v3.73.0, E2)<!-- origin:end -->: WHEN the handoff state carries `dispatch_mode: "bugfix"` → BEFORE writing any fix, DO write the reproduction test, run it, and confirm it is RED (fails against the un-fixed code); then record it in `qa_reports/expected-red_<active_feature>.txt` (`<relative test file path> | <exact test name>` per line, the same C15 format as step 7a). Only then implement the fix. The server BLOCKS your `sr-engineer:In_Progress → code-reviewer:In_Progress` handoff with `REPRO_MANIFEST_MISSING` until that manifest exists — the repro-first discipline is enforced, not advisory. **AC6 escape**: if repro is genuinely infeasible (the "bug" needs no red-first proof, or no reproduction can be constructed), do NOT fake a manifest — escalate `status=Blocked` to pm per *Escalation Routes: awaiting clarification* (the Blocked edge is never gated). ELSE (feature mode / absent `dispatch_mode`) continue — this step is a no-op.
