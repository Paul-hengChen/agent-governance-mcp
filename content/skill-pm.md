...
- **Acceptance Criteria** — BDD: `Given / When / Then`. Each AC must be testable. **`proof:` annotation**<!-- origin:start --> (v3.77.0, E3)<!-- origin:end -->: where feasible — i.e. when an individual AC is provable by ONE command, ONE test (name), or ONE pixel-diff region — the AC carries an indented `proof:` line (first non-whitespace token of its own line, directly under the AC) naming that exact command/test-id/region; qa-engineer executes it verbatim at review time. This is a per-AC judgment, NOT a blanket requirement: a genuinely subjective AC (a UX judgment call with no single-command truth) MAY omit `proof:` — do not force fake automation onto criteria that cannot honestly carry it. Worked example:
  ```markdown
  - **AC1** — Given the CLI is installed, when `mycli --version` runs, then it prints the package.json version and exits 0.
    proof: `mycli --version; echo "exit=$?"` prints the package.json version followed by `exit=0`.
  ```
...
<!-- rationale:start -->
Reason: implementations otherwise paraphrase from requirement prose, which silently drifts from the design (a prior rollout shipped `"Select your language"` because nobody pinned the Figma title `"Language"` in this section).<!-- rationale:end -->
...
<!-- rationale:start -->
Reason: stylistic ACs ("font 32 sp / 700 / `#FFFFFF`") only catch what the spec already enumerates; an unsourced hex slipping into `OobeTheme.kt` is exactly the kind of silent drift that ate a prior visual rollout.<!-- rationale:end -->
...
8. `tw_update_state(active_feature=<name>, status=In_Progress, next_role="architect" or "sr-engineer", pending_notes=[...])` — `next_role` is the first-class routing field (schema v7), enum-validated by the server; `pending_notes` is free prose for the next reader. Include `cut_approved: true` only per the Cut-Approval Gate dispatch branch (Gate Summary; full mechanism and trust rule: Constitution §3.1). Decide architect vs sr-engineer based on complexity (≥ 3 modules, new data model, or cross-cutting API → architect).

**Bugfix mode**<!-- origin:start --> (v3.73.0, E2)<!-- origin:end --> (small bug-fix ticket): when the work is a bug fix rather than a new feature, set `dispatch_mode: "bugfix"` on your step-8 `tw_update_state` write and route `next_role: "sr-engineer"` — the default bugfix chain is pm → sr-engineer → code-reviewer → qa-engineer, with architect and design-auditor skipped (AC1). Bugfix mode makes the repro-first discipline load-bearing: sr-engineer must record a failing reproduction test before the fix (server-enforced), and QA's PASS requires that declared red-set turned green with zero unexplained reds. **AC4 opt-back-in**: if a "bug" turns out cross-cutting (≥3 modules, a data-model change, or a cross-cutting API — the same threshold that routes to architect), set `dispatch_mode: "feature"` and/or route to architect instead — bugfix mode only changes the *default* chain, it never blocks legitimate design work. Feature-scoped: the mode carries across the chain for the same `active_feature` and is NOT re-armed on your re-entry; change it only by an explicit write. Example cut line: `- [ ] T-BUG-01 [P0] sr-engineer: fix <symptom> — repro-first (record failing test in qa_reports/expected-red_<feature>.txt before the fix) | depends_on: none`.

**Amend-Resume declaration** (mid-chain spec amendment only): when your `pm:In_Progress` write amends a spec-only issue and resumes a specific stranded downstream role (`code-reviewer` or `qa-engineer`), set the first-class `resume_of` field to that role (alongside `next_role` naming the same role) as the resume declaration — the server accepts the resume edge only when the field names the exact target role; a legacy `resume_of:` line inside `pending_notes` is inert. Full mechanism: Constitution §3.1.
...
| **Resource Audit Gate** (constitution §7) | Always — scan every supplied requirement document for external references: grep at minimum for `http(s)://`, `figma`, `sketch`, `mockup`, `設計圖`, `URL`, `link`, `see <ticket>`, `Azure DevOps`, `JIRA`. | For EACH hit, the reference is presumed load-bearing — classify as `fetch / index / ignore` and record it as an `external_refs` entry (state `fetched` / `indexed` / `user-confirmed-ignorable`) on your `tw_update_state` write; passing the field REPLACES the array wholesale. An entry left `unresolved` BLOCKS your step-8 routing write into build (`EXTERNAL_REFS_UNRESOLVED`); omitting a known-load-bearing ref entirely defeats the gate — do neither. Zero hits → omit the field (absence = non-blocking). Do NOT let architect or sr-engineer silently defer one. |
| **Source-Credibility Gate** (E4) | Awareness only — you do NOT author this attestation; the design-auditor records `credibility: full-page-composite` on each `audited` `## Source` row (its step 2b). For a fetch-based `design/<feature>.md` (`## Mode` ∈ `figma`/`sketch`/`xd`/`penpot`), a missing/wrong attestation BLOCKS your step-8 handoff into build with `SOURCE_CREDIBILITY_UNVERIFIED`. | Do NOT hand-edit the design file — route back to `design-auditor` to re-run step 2b and record the attestation, then retry the build handoff. See `specs/e4-design-source-credibility-gate.md`. |
