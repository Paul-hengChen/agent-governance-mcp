...
4. **External-reference Sanity Gate**: cross-check `Deferred Resources` against the spec's *Dependencies / Prerequisites*. If you find a reference in the spec that is NOT in `Deferred Resources` AND was NOT fetched, → block with `["Architect blocked: external reference '<ref>' not classified by PM", "next_role: pm"]`. Constitution §7 forbids unilateral defer.
