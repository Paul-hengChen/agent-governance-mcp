...
Staging scope includes ALL uncommitted upstream work, not just files you edited this turn: stage these directories `lib/ content/ templates/ specs/ test/ qa_reports/ review_reports/` plus metadata `package.json index.ts CHANGELOG.md README.md dist/`. Run `git diff --cached --stat` and verify source directories appear before committing.
