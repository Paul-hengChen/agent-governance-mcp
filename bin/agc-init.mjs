...
//
//   agc init   Scaffold .current/.config.json, tasks.md, and per-agent entry
//              adapters (CLAUDE.md, AGENTS.md, .antigravityrules). Idempotent:
//              existing files are skipped; the CLAUDE.md adapter block is
//              upserted in place. Deliberately does NOT write
//              .current/handoff.md (E34): any seeded prev tuple dead-ends the
...
const STR_USAGE =
  "Usage: agc <command>\n" +
  "  init    Scaffold .current/.config.json, tasks.md, and per-agent entry\n" +
...
function runInit(cwd) {
  // NOTE (E34): no .current/handoff.md is scaffolded here. The transition
...
`;
  ...
  const files = [
    // .current/ itself is created by this entry's mkdirSync below.
