...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (!args || typeof args.workspace_path !== "string") {
        return {
            content: [{ type: "text", text: "❌ Missing required parameter: workspace_path" }],
            isError: true,
        };
    }
    const wp = args.workspace_path;
    try {
        switch (name) {
            ...
            case "sdd_update_state": {
                ...
                const result = writeHandoffState(wp, args.active_feature, args.status, args.completed_tasks || [], args.pending_notes || []);
                return { content: [{ type: "text", text: result }] };
            }
            case "sdd_complete_task": {
                enforcePreFlight(wp, "sdd_complete_task");
                const result = completeTask(wp, args.task_id, args.note);
                return { content: [{ type: "text", text: result }] };
            }
            case "sdd_rollback_task": {
                enforcePreFlight(wp, "sdd_rollback_task");
                const result = rollbackTask(wp, args.task_id, args.reason);
                return { content: [{ type: "text", text: result }] };
            }
            default:
                return {
