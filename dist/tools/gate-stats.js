...
export function computeGateStats(workspacePath) {
    ...
    if (!tel.exists)
        caveats.push(`No telemetry yet: ${telemetryPath} not found.`);
    if (!met.exists)
        caveats.push(`No metrics yet: ${metricsPath} not found.`);
    return {
