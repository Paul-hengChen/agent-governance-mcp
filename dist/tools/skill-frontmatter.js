...
export function parseSkillFile(text) {
    ...
    try {
        ...
        process.stderr.write(`[skill-frontmatter] malformed YAML frontmatter — ignoring: ${err.message}\n`);
    ...
    if (raw && typeof raw === "object" && !Array.isArray(raw)) {
        ...
        if ("recommended_model" in rec) {
            if (isModelTier(rec.recommended_model)) {
                ...
                process.stderr.write(`[skill-frontmatter] invalid recommended_model value ${JSON.stringify(rec.recommended_model)} — expected one of ${MODEL_TIERS.join("/")}\n`);
            }
        }
    }
    return { frontmatter, body };
