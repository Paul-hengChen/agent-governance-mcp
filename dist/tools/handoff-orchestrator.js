...
function effectiveAllowedSuccessors(prev, counters, options = {}) {
    ...
    for (const agent of ALL_AGENT_NAMES) {
        for (const status of ALL_STATUS_NAMES) {
            const accepted = featureChangedBranches.some((feature_changed) => validateTransition({
                prev,
                next: { agent, status },
                ...
                feature_changed,
                next_resume_of: assumeResumeOf && (agent === "code-reviewer" || agent === "qa-engineer")
                    ? agent
                    : undefined,
            }) === null);
            if (accepted)
...
async function handleUpdateStateCore(parsed) {
    ...
    if (parsed.next_role) {
        ...
        if (!isLegalSuccessor) {
            ...
            nextRoleWarnings.push(`next_role="${parsed.next_role}" does not match any allowed successor of the state ` +
                `just written (${nextTuple.agent}:${nextTuple.status}). Actual allowed next ` +
                `(agent:status) pair(s): ${allowedPairs.length > 0
                    ? allowedPairs.join(", ")
