...
test("AC-9: package.json version field equals 3.50.0", () => {
  ...
  // must be "3.50.0" for the release to be self-consistent. Updated from 3.49.0
  // to 3.50.0 when a13-section1-polish shipped.
...
test("AC-9: index.ts Server() literal equals 3.50.0", () => {
  ...
  // to MCP clients. It must match package.json. Updated from 3.49.0 to 3.50.0 when
  // a13-section1-polish shipped.
  const srcIndex = fs.readFileSync(
    path.join(path.dirname(new URL(import.meta.url).pathname), "..", "index.ts"),
    "utf-8",
  );
  assert.match(srcIndex, /Server\(\s*\{[^}]*version:\s*["']3\.50\.0["']/s,
