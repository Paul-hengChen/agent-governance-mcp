...
test("AC-9: package.json version field equals 3.49.0", () => {
  ...
  // must be "3.49.0" for the release to be self-consistent. Updated from 3.48.0
  // to 3.49.0 when c13-release-engineer-write-path shipped.
...
test("AC-9: index.ts Server() literal equals 3.49.0", () => {
  ...
  // to MCP clients. It must match package.json. Updated from 3.48.0 to 3.49.0 when
  // c13-release-engineer-write-path shipped.
  const srcIndex = fs.readFileSync(
    path.join(path.dirname(new URL(import.meta.url).pathname), "..", "index.ts"),
    "utf-8",
  );
  assert.match(srcIndex, /Server\(\s*\{[^}]*version:\s*["']3\.49\.0["']/s,
