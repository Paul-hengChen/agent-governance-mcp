...
test("AC-9: package.json version field equals 3.42.0", () => {
  // Why: AC-9 pins the version contract. Both package.json and the Server() literal
  // must be "3.42.0" for the release to be self-consistent. Updated from 3.40.1
  // to 3.42.0 when qa-visual-pixel-gate-attestation (seventh visual sub-gate) shipped.
  const pkgPath = path.join(path.dirname(new URL(import.meta.url).pathname), "..", "package.json");
  const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf-8"));
  assert.equal(pkg.version, "3.42.0", `package.json version must be "3.42.0", got "${pkg.version}"`);
});

test("AC-9: index.ts Server() literal equals 3.42.0", () => {
  // Why: the Server() version literal in index.ts is the operational version broadcast
  // to MCP clients. It must match package.json. Updated from 3.40.1 to 3.42.0 when
  // qa-visual-pixel-gate-attestation (seventh visual sub-gate) shipped.
  const srcIndex = fs.readFileSync(
    path.join(path.dirname(new URL(import.meta.url).pathname), "..", "index.ts"),
    "utf-8",
  );
  assert.match(srcIndex, /Server\(\s*\{[^}]*version:\s*["']3\.42\.0["']/s,
    "Server() version literal in index.ts must equal 3.42.0");
});
