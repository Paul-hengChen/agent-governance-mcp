...
test("AC-1: writeHandoffState stamps schema_version: 7 in YAML (c9-protocol-fields)", async () => {
  ...
  // c9-protocol-fields bump: handoff schema is now 7 (added next_role/resume_of/
  // review_verdict, stamp-only migration). b8-external-ref-ledger had added
  // external_refs (=6).
  assert.match(content, /schema_version:\s*7/);
});
...
test("AC-2: readHandoffState heals v0 handoff to CURRENT (v7) on disk (fire-and-forget)", async () => {
  const ws = mkWorkspace();
  resetSession();
...
`,
  ...
  // c9-protocol-fields: chain climbs v0→v1→v2→v3→v4→v5→v6→v7; healed file lands at CURRENT (=7).
  assert.match(healed, /schema_version:\s*7/);
});
...
test("AC-2 regression: existing handoff missing schema_version round-trips to CURRENT (v7)", async () => {
  const ws = mkWorkspace();
  resetSession();
  writeRaw(
    ws,
...
`,
  ...
  // c9-protocol-fields: v0 → v1 → ... → v6 → v7 chain lands at CURRENT.
  assert.match(read(ws), /schema_version:\s*7/);
});
...
`,
  ...
  assert.throws(
    () => readHandoffState(ws),
    /handoff on-disk version 99 > server max 7/,
  );
});
...
`,
  ...
  assert.throws(
    () => parseHandoff(ws),
    /on-disk version 42 > server max 7/,
  );
});
...
`,
  ...
  // c9-protocol-fields: chain lands at CURRENT (=7).
  assert.match(read(ws), /schema_version:\s*7/);
});
