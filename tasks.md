...
- [ ] T28 [P0] Wire handoff.md migrations: add `schema_version` to YAML frontmatter in `tools/handoff.ts`, register v0→v1 migration (absent → 1), call `runMigrations()` on read path, atomic write-back via existing tmp+rename + `withFileLock` (AC-1, AC-2, AC-5). | depends_on: T27
- [ ] T29 [P0] Wire tasks.md migrations: persist version via leading HTML comment `<!-- schema_version: N -->` in `tools/tasks-file.ts`, register v0→v1 migration, run on every parse, atomic rewrite preserving checkbox state (AC-1, AC-2). | depends_on: T27
- [ ] T30 [P0] Wire SQLite migrations in `tools/storage-sqlite.ts`: add `schema_meta(kind TEXT PRIMARY KEY, version INTEGER)` table, seed `(sqlite, 1)` on init, run pending migrations inside a transaction at first query after open, refuse-loud on future version (AC-1, AC-2, AC-4). | depends_on: T27
- [ ] T31 [P1] Wire `.current/.config.json` migrations in `tools/config.ts`: add `schema_version` JSON key, register v0→v1 migration (absent → 1), upgrade on first read, atomic write-back (AC-1, AC-2). | depends_on: T27
- [ ] T32 [P1] Extend `tw_detect_drift` in `tools/drift.ts` to surface version skew across the four artifacts as a distinct drift reason, separate from completed-task drift (AC-6). | depends_on: T28, T29, T30, T31
- [ ] T33 [P2] Document the migration framework: add `docs/schema-versions.md` (how to author a new migration: bump CURRENT, add Migration object, fixtures), and update `CLAUDE.md` "What this server does" to mention schema versioning. No code changes. | depends_on: T32
