...
| 8c | **E68** | blocked — needs E48's design decision first | Filed from the same v3.101.0 release but deliberately NOT batched with 8b. `docs/skills/release-engineer.md` has now been touched by three consecutive cuts under three different carve-outs and is internally inconsistent as a result. Syncing it is cheap; syncing it *before* deciding what `docs/skills/*` is (E48 / `.current/feature-split.md` F1: delete-and-link vs stale-quote guard vs accept-unchecked) risks doing work the decision discards. Sequence it immediately after that decision, whichever way it goes |
