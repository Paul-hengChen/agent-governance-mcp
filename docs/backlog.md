...
| E14 | CI-status self-report vs. ground-truth: release-engineer currently asserts "npm test green" by self-check; optional follow-on reads `gh` API to verify CI status before release — deferred from E7 per spec Out of Scope, enables automated release gates on CI health. **done (2026-07-13, v3.83.0)** — Check 6 added to `scripts/verify-release.mjs` (queries latest completed CI run on origin/main via `gh`, FAILs on non-success, degrades gracefully on gh CLI missing/unauthenticated); integrated into release-engineer SOP step 9a with live exercise at release time; tag v3.83.0 (commit cbe0d21). | P3 | E7 ✓ | ~2 (release SOP optional step, gh API call, test) | — |
...
| E16 | `ALLOWED_TRANSITIONS` has no native `pm → qa-engineer` intake edge for a single-role test-only ticket (E15 run had to enter via the Amend-Resume edge, whose documented purpose is narrower — disclosed honestly by qa-engineer in review_T-E15-01.md) — either add a sanctioned direct edge (maybe gated on a scope attestation) or document Amend-Resume as the blessed door for this shape. **done (2026-07-13, v3.83.0)** — Constitution §3.1 charter broadening: qa-engineer:In_Progress Amend-Resume entry now requires `resume_of` (matching code-reviewer:In_Progress direct entry semantics); content amendments in const-08-chain-31-mid.md (two new bullets) + coord-03-core-fallback.md (coordinator pointer); tools amendment: handoff-orchestrator.ts resume_of gate predicate extended; test/e16-judge-dispatch-charter.test.mjs (new suite, golden fixtures regenerated); 1420/1420 tests green; tag v3.83.0 (commit cbe0d21). | P3 | — | ~2 (tools/transitions.ts edge + const §3.1 note + test) | — |
