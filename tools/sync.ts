...
export async function reconcileTasks(workspacePath: string): Promise<string> {
  ...
  const handoffCompleted = new Set<string>();
  for (const id of vocab) {
    const re = new RegExp(`\\b${escapeRegExp(id)}\\b`);
    if (handoff.completed_tasks.some((c) => re.test(c))) handoffCompleted.add(id);
  }
