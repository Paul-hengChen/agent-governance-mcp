...
function isOriginAllowed(originHeader: string | undefined, allowlist: string[]): boolean {
  if (!originHeader) return true; // No Origin → not a browser-driven request; allow.
  if (allowlist.length === 0) return true; // No allowlist configured → allow.
